...
//        int res=test.myAtoi("-91283472332");
    ...
    public int myAtoi(String s) {
        ...
        s=s.trim();//去掉空格
        boolean bignum=true;//正负数  true 正数

        List<Character> list=initList();
        ...
        try {
            ...
            if(temp.contains(".")){//小数处理
                Double dnum=new Double(temp);
                num= dnum.intValue();
                System.out.println("double"+dnum.intValue());
            }
            Long lnum=new Long(temp);
            if(lnum>Integer.MAX_VALUE){
                num = 0;
            }else if(lnum<Integer.MIN_VALUE) {
